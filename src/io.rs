use crate::entity::{SaveTimePeriod, TSCacheValue, TSItem, TSValue};
...
impl FileIOCache {
    ...
    fn create_new_file(&mut self) {
        ...
        if self.write.is_none() {
            ...
            self.write = Some(BufWriter::new(
                File::create(format!("{}/{}.tc", dir, time)).unwrap(),
            ));
        }
    }
    ...
    pub fn append(&mut self, value: &TSValue) {
        ...
        match self.write {
            Some(ref mut w) => {
                let mut buff = vec![];
                buff.write_u64::<BigEndian>(value.key).unwrap();
                buff.write(to_vec_named(&value.value).unwrap().as_slice())
                    .unwrap();
                w.write_all(&buff).unwrap();
            }
            _ => {}
        }
        let period = self.ts_item.storageEnum.as_period() * 1000;
        let time = SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap()
            .as_millis();
...
pub fn write_all_items(items: &Vec<&TSItem>) {
    ...
    let mut file = OpenOptions::new()
        ...
        .open(path)
        .unwrap();
    file.write_all(to_vec_named(&items).unwrap().as_slice())
        .unwrap();
}
